...
import { useDeleteTrip, useTrip, useUpdateTrip } from "@/src/hooks/useTrips";
...
import {
  useCreatePassenger,
  useOwnerUpdatePassenger,
} from "@/src/hooks/usePassengers";
import { CreatePassenger, TripPassengerFormat } from "@/src/utils/types";
...
const TripDetailsPage = () => {
  ...
  const [updatedTripInformation, setUpdatedTripInformation] = useState(null);
  ...
  const { mutateAsync: ownerUpdatePassenger } = useOwnerUpdatePassenger(
    updatePassenger,
    trip?.id,
    status,
    pending
  );
  const { mutateAsync: deleteTrip } = useDeleteTrip(id, user?.uid);
  const { mutateAsync: updateTrip } = useUpdateTrip(
    id,
    updatedTripInformation,
    user?.uid
  );
  ...
  useEffect(() => {
    ...
    if (
      ...
      trip?.data?.vehicle.seats_occupied
    ) {
      if (
        trip.data.vehicle.seats_available - trip.data.vehicle.seats_occupied ===
        0
      ) {
        setFull(true);
      }
    ...
    if (trip?.data?.passengers) {
      if (
        trip.data.passengers.some(
          (passenger) => passenger.driver.id === user?.uid
        )
      ) {
  ...
  const handleEditTrip = () => {
    if (trip.data.vehicle.seats_occupied == 0) {
      setShowEditModal(true);
    } else {
      Alert.alert("Failed", "Trip already has passengers, cannot edit.", [
        {
          text: "OK",
        },
      ]);
  ...
  const handleUpdateTrip = (tripInfo) => {
    setUpdatedTripInformation(tripInfo);
  ...
  const handleRequestToJoinModal = (coordinates: {
    latitude: string;
    longitude: string;
    address: string;
  }) => {
  ...
  return (
    <SafeAreaView style={styles.container}>
      <ScrollView contentContainerStyle={styles.scrollContainer}>
        ...
        {!active ? <Text style={styles.inactiveText}>Inactive</Text> : null}
        ...
        <View style={styles.infoRow}>
          ...
          <Text style={styles.infoText}>Time: {trip.data.time_of_trip}</Text>
        </View>
        ...
        {/* Button to Show Trip on Map */}
        <TouchableOpacity
          style={styles.showMapButton}
          onPress={() => setIsModalVisiblePassenger(true)}
        >
          <Text style={styles.showMapButtonText}>Show Trip on Map</Text>
        </TouchableOpacity>
        ...
        <View style={styles.infoRow}>
          ...
          <Text style={styles.infoText}>
            Driver: {trip.data.driver.username} {isOwner ? "(You)" : ""}
          </Text>
        </View>
        ...
        <Text style={styles.sectionTitle}>
          {isOwner ? "Accepted Passengers" : "Passengers"}
        </Text>
        {trip.data.passengers.filter(
          (p) => !p.pending && p.status === "confirmed"
        ).length > 0 ? (
          trip.data.passengers
            .filter(
              (passenger) =>
                passenger.pending == 0 && passenger.status === "confirmed"
            )
            .map((passenger) => (
              <View key={passenger.id} style={styles.passengerContainer}>
                <Text style={styles.passengerText}>
          ...
          <Text style={styles.noPassengersText}>
            {isOwner ? "No accepted passengers yet." : "No passengers yet."}
          </Text>
        )}
      ...
      <MapWithRouteModal
        ...
        middleMarkers={trip.data.passengers
          .filter(
            (p) =>
              p.pending === 0 &&
              (!selectedPassenger || p.id !== selectedPassenger.id)
          ) // Exclude selected passenger
          .map((p) => ({
